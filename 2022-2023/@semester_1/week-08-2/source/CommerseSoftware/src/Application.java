...
public class Application {
    ...
    public static void processReport(Trader traderManager) {
        ...
        Terminal.output("Отчет покупки");
        Terminal.output("Отчет продажби");
        Terminal.output("Отчет текущ баланс");
        int reportId =  Terminal.input();
        ...
        if(reportId == 2) {
            traderManager.sellReport();
        }

        if(reportId == 3) {
            traderManager.balanceReport();
        }
    }
